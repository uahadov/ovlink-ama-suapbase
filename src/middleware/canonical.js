const { getSafeHostHeader, normalizeHostName, isLocalOrPrivateHost } = require('../lib/url-helpers');
...
function httpsRedirectMiddleware(req, res, next) {
  ...
  if (isLocalOrPrivateHost(hostHeader)) return next();
  if (!req.secure) {
...
function canonicalHostMiddleware(req, res, next) {
  ...
  if (isLocalOrPrivateHost(currentHostOnly)) {
    return next();
  }
