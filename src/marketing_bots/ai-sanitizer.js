...
function sanitizeAIEmail(rawText, lead = {}) {
  ...
  if (subjectMatch) {
    ...
    const greetingMatch = text.match(/(?:^|\n)\s*(?:Hi\b|Hello\b|Dear\b|Hey\b)[\s\S]*/iu);
    if (greetingMatch) {
  ...
  const postCheckPatterns = [
    /(?:\r?\n)+\s*(?:That's \d+ sentence|That is \d+ sentence|This is \d+ sentence|Let me check:?|Sentence \d+:|Wait, that last one|Word count:|Hope this helps!?|Here's the breakdown:?|Rationale:?|Explanation:?|Self-evaluation:?|Analysis:?|Notes?:?)(?:[\s\S]*)$/i,
  ...
  body = body.replace(/^(?:The user wants me to|Here's a thinking process|Let me craft|Constraints:)[\s\S]*?(?=(?:Hi|Hello|Dear|Hey)\b)/iu, '');
  ...
  if (/(?:That(?:'s|\s+is)|This\s+is)\s+\d+\s+sentence/i.test(body) || /(?:That(?:'s|\s+is)|This\s+is)\s+\d+\s+sentence/i.test(subject)) {
    return null;
  }
...
function sanitizeAISocialReply(rawText) {
  ...
  const postCheckPatterns = [
    /(?:\r?\n)+\s*(?:That's \d+ sentence|That is \d+ sentence|This is \d+ sentence|Let me check:?|Sentence \d+:|Wait, that last one|Word count:|Hope this helps!?|Notes?:?)(?:[\s\S]*)$/i
  ];
  for (const pattern of postCheckPatterns) {
  ...
  if (/(?:That(?:'s|\s+is)|This\s+is)\s+\d+\s+sentence/i.test(text)) {
    return null;
  }

  // Must be between 30 and 1500 characters
