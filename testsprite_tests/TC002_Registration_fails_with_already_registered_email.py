...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /register (use the explicit navigate action to http://localhost:3000/register).
        await page.goto("http://localhost:3000/register", wait_until="commit", timeout=10000)
        
        # -> Type the provided email into the email field (index 823) as the immediate action.
        ...
        frame = context.pages[-1]
        frame = context.pages[-1]
        error_locator = frame.locator('xpath=/html/body/main/section/div/div[2]')
        await error_locator.wait_for(state='visible', timeout=5000)
        text = await error_locator.text_content()
        assert text is not None and "This email is already in use." in text, "Expected error message 'This email is already in use.' to be visible on the page"
